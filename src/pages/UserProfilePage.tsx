...
export function UserProfilePage() {
  ...
  useEffect(() => {
    if (!me?.id || !userId || me.id === userId) return;
  ...
  useEffect(() => {
    ...
    /* If private + not following, don't load posts */
    if (otherProfile.is_private && !following && !isOwnProfile) return;
    setPostsLoading(true);
    supabase.from("posts").select(POST_SELECT)
      .eq("user_id", userId).eq("is_forward", false).is("deleted_at", null)
      .order("created_at", { ascending: false })
      .then(({ data }) => { if (data) setQuotes(data.map(buildPost)); setPostsLoading(false); });
  }, [userId, otherProfile, following, isOwnProfile]);

  /* ── Load forwards ── */
  const loadForwards = useCallback(async () => {
    if (!userId || fwdLoaded) return;
    const { data } = await supabase.from("posts").select(POST_SELECT)
      .eq("user_id", userId).eq("is_forward", true).is("deleted_at", null)
      .order("created_at", { ascending: false });
  ...
  const handleFollow = async () => {
    ...
    if (following) {
      setFollowing(false);
      setOtherProfile(p => p ? { ...p, followers_count: Math.max(0, p.followers_count-1) } : p);
      await supabase.from("follows").delete().eq("follower_id", me.id).eq("following_id", userId);
    } else {
      setFollowing(true);
      setOtherProfile(p => p ? { ...p, followers_count: p.followers_count+1 } : p);
      await supabase.from("follows").insert({ follower_id: me.id, following_id: userId });
      /* Notify */
      await supabase.from("notifications").insert({ user_id: userId, actor_id: me.id, type: "follow" }).catch(()=>{});
    }
  };
  ...
  const handleDM = async () => {
    ...
    const { data: existing } = await supabase.from("conversations")
      .select("id")
      .or(`and(participant_1.eq.${me.id},participant_2.eq.${userId}),and(participant_1.eq.${userId},participant_2.eq.${me.id})`)
      .maybeSingle();
    if (existing?.id) { navigate("/messages"); return; }
    await supabase.from("conversations").insert({ participant_1: me.id, participant_2: userId });
    navigate("/messages");
  };
  ...
  const handleBlock = async () => {
    if (!me?.id || !userId) return;
    await supabase.from("blocks").upsert({ blocker_id: me.id, blocked_id: userId }, { onConflict: "blocker_id,blocked_id", ignoreDuplicates: true });
    setShowMenu(false);
    navigate(-1);
  };
  ...
  const isPrivateLocked = otherProfile.is_private && !following && !isOwnProfile;
      ...
      <div className="min-h-screen bg-white dark:bg-gray-950">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10">
          ...
          <div className="text-center">
            <p className="font-bold text-gray-900 dark:text-white text-sm">{otherProfile.name}</p>
            <p className="text-xs text-gray-400">{otherProfile.posts_count} quotes</p>
          </div>
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-11 z-40 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden min-w-[160px]">
                  ...
                  <button onClick={() => { setShowMenu(false); navigate(`/report/${userId}`); }}
        ...
        <div className="h-32 bg-gradient-to-r from-blue-500 to-purple-600 overflow-hidden">
          {otherProfile.banner_url && <img src={otherProfile.banner_url} alt="" className="w-full h-full object-cover"/>}
        </div>

        {/* Avatar + action buttons */}
        <div className="px-4 relative -mt-10 flex items-end justify-between mb-3">
          {/* Avatar */}
          {otherProfile.avatar_url && !avatarErr ? (
            <img src={otherProfile.avatar_url} alt={otherProfile.name} onError={() => setAvatarErr(true)}
            ...
            <div className="w-20 h-20 rounded-full bg-blue-600 ring-4 ring-white dark:ring-gray-950 shadow-sm flex items-center justify-center">
              <span className="text-white text-3xl font-extrabold">{otherProfile.name[0]?.toUpperCase()||"U"}</span>
            </div>
          )}
          {/* Buttons */}
          {!isOwnProfile && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <button onClick={handleFollow}
                className={`flex items-center gap-1.5 px-5 py-2 rounded-full text-sm font-bold transition-all ${
                  following
                    ? "border-2 border-gray-200 dark:border-gray-700 text-gray-800 dark:text-white hover:border-red-300 dark:hover:border-red-700 hover:text-red-500"
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2 mb-0.5">
            <h1 className="text-xl font-extrabold text-gray-900 dark:text-white">{otherProfile.name}</h1>
            ...
            {otherProfile.is_private && (
              <Lock size={14} className="text-gray-400"/>
            )}
          </div>
          <p className="text-gray-400 text-sm mb-2">@{otherProfile.username}</p>
          {otherProfile.bio && <p className="text-gray-800 dark:text-gray-200 text-sm leading-relaxed mb-2">{otherProfile.bio}</p>}
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400 mb-3">
            {otherProfile.location && <span className="flex items-center gap-1"><MapPin size={12} className="text-gray-400"/>{otherProfile.location}</span>}
            {otherProfile.website_url && (
              <a href={otherProfile.website_url} target="_blank" rel="noreferrer"
                className="flex items-center gap-1 text-blue-600 dark:text-blue-400 hover:underline">
                <Link2 size={12}/>{otherProfile.website_label||otherProfile.website_url}
              </a>
            )}
          </div>
          <div className="flex gap-4 text-sm">
            <button onClick={() => navigate(`/user/${userId}/followers`)} className="hover:underline">
              <span className="font-bold text-gray-900 dark:text-white">{(otherProfile.followers_count||0).toLocaleString()}</span>
              <span className="text-gray-500 dark:text-gray-400 ml-1">Followers</span>
            </button>
            <button onClick={() => navigate(`/user/${userId}/followers`)} className="hover:underline">
              <span className="font-bold text-gray-900 dark:text-white">{(otherProfile.following_count||0).toLocaleString()}</span>
        ...
        {isPrivateLocked ? (
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <button onClick={handleFollow}
              className="mt-4 bg-blue-600 text-white px-6 py-2.5 rounded-full font-bold text-sm hover:bg-blue-700 transition-colors">
            ...
            {activeTab==="quotes" && (
              postsLoading
                ...
                ? <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">No quotes yet</p>
                  </div>
            ...
            {activeTab==="forwards" && (
              forwards.length===0
                ? <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">No forwards yet</p>
                  </div>
